...
const PawnPositionFns: Record<
  Color,
  { attack: [GetSiblingFn, GetSiblingFn]; move: GetSiblingFn }
> = {
  white: {
    attack: [getLeftTopSibling, getRightTopSibling],
    move: getTopSibling,
  },
  black: {
    attack: [getLeftBottomSibling, getRightBottomSibling],
    move: getBottomSibling,
  },
};
...
export const getPawnAvailablePositions = ({
  ...
  if (!isAttackOnly) {
    ...
    if (siblingPosition && !board[siblingPosition]) {
      ...
      const hasNotMoved =
        (pawnColor === 'white' && rowIndex === WHITE_PAWN_START_ROW) ||
